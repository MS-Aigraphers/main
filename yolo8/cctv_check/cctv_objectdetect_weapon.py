    ...
    while True:
        ...
        for i, (cls, bbox) in enumerate(zip(classes_detect, bboxes_detect)):
            ...
            if class_name == "6: Weapon" and confidence < 0.7:
                continue
            if class_name == "0 : Human" and confidence < 0.3:
                continue
            if class_name == "5 : Bag" and confidence < 1 :
                continue
            if confidence < 0.5:
                continue
        ...
        if iou == 0 and recent_paying > paying_threshold and paying == 0:
            ...
            for value, count in value_counts.items():
                ...
                if value == max(values):
                    ...
                    # max_percentage_object_class보다 큰 값을 가진 클래스 중 백분율이 20% 이상인 클래스 찾기
                    max_percentage_object_class_int = int(
                        max_percentage_object_class)  # max_percentage_object_class를 정수로 변환
                    threshold_percentage = 20  # 원하는 백분율 임계값 설정 (예: 20%)

                    # 20% 이상인 클래스 중 가장 큰 백분율을 가진 객체 클래스 찾기
                    max_above_threshold_class = None
                    max_above_threshold_percentage = 0
                    ...
                    # 만약 20% 이상인 클래스 중 가장 큰 백분율을 가진 객체 클래스가 없으면 가장 높은 백분율을 출력
